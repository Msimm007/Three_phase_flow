#ifndef SV_PROBLEM_MID_HH
#define SV_PROBLEM_MID_HH
...
namespace VaporSaturationMidpoint
{
...
template <int dim>
class VaporSaturationProblem_midpoint
{
public:
	VaporSaturationProblem_midpoint(Triangulation<dim, dim> &triangulation_,
			...
			bool use_direct_solver_, bool Stab_v_, bool project_Darcy_with_gravity_,
...
template <int dim>
VaporSaturationProblem_midpoint<dim>::VaporSaturationProblem_midpoint(Triangulation<dim, dim> &triangulation_,
		const unsigned int degree_, double time_step_, double theta_n_time_,
		double theta_Sv_, double penalty_Sv_,
		double penalty_Sv_bdry_, std::vector<unsigned int> dirichlet_id_sv_, bool use_exact_pl_in_Sv_,
		bool use_exact_Sa_in_Sv_, double time_, unsigned int timestep_number_,
...
template <int dim>
void VaporSaturationProblem_midpoint<dim>::setup_system()
{
...
template <int dim>
void VaporSaturationProblem_midpoint<dim>::assemble_system_matrix_vapor_saturation()
{
	setup_system();
								 ...
								 auto & copy_data)
	{
		...
		std::vector<Tensor<1, dim>> old_Sv_grads_n(n_qpoints);
		...
		fe_v.get_function_values(temp_pl_solution_kplus1, pl_vals);
		fe_v.get_function_gradients(temp_pl_solution_kplus1, pl_grads);

		fe_v.get_function_values(temp_Sa_solution_kplus1, Sa_vals);
...
//			double rho_v_nminus1 = rho_v_fcn.value(pl_value_nminus1, Sa_value_nminus1, Sv_value_nminus1);
			...
			for (unsigned int i = 0; i < n_dofs; ++i)
			{
				...
				if(timestep_number == 1 || !second_order_time_derivative)
				{
					copy_data.cell_rhs(i) += (1.0/(theta_n_time*time_step)) * phi_nplus1 * Sv_value_n
...
//											* fe_v.shape_value(i, point) * JxW[point];
				...
				if (Stab_v)
                {
                    copy_data.cell_rhs(i) += (-lambda_v * dpcv_dSv + Kappa_tilde_v)
                                             * kappa * Sv_grad_n
				...
				if(project_only_kappa)
					copy_data.cell_rhs(i) += (lambda_v) * totalDarcyVelo_extrapolation
							* fe_v.shape_grad(i, point) * JxW[point];
				else
					copy_data.cell_rhs(i) += (lambda_v/lambda_t) * totalDarcyVelo_extrapolation
							* fe_v.shape_grad(i, point) * JxW[point];

				// Gravity term
				if(!project_Darcy_with_gravity)
					copy_data.cell_rhs(i) += kappa*rho_v*lambda_v
									 ...
									 auto & copy_data)
	{
		...
		fe_face.get_function_values(temp_pl_solution_kplus1, pl_vals);
		fe_face.get_function_gradients(temp_pl_solution_kplus1, pl_grads);

		fe_face.get_function_values(temp_Sa_solution_kplus1, Sa_vals);
		...
		if(dirichlet)
		{
			for (unsigned int point = 0; point < n_qpoints; ++point)
			{
				...
				double gamma_Sv_e = fabs(lambda_v*kappa*dpcv_dSv);
				...
				for (unsigned int i = 0; i < n_facet_dofs; ++i)
				{
                    ...
                    if (Stab_v)
                    {
                        copy_data.cell_rhs(i) += (lambda_v*dpcv_dSv - Kappa_tilde_v) // added to RHS
                                                 * kappa
                                                 * Sv_grad_k
                                                 * normals[point]
                                                 * fe_face.shape_value(i, point)
					...
					if(project_only_kappa)
						...
						copy_data.cell_rhs(i) -= (lambda_v/lambda_t)
												* totalDarcyVelo_extrapolation
												* normals[point]
												* fe_face.shape_value(i, point)
												* JxW[point];
								 ...
								 auto & copy_data)
	{
		...
		fe_face.get_function_values(temp_pl_solution_kplus1, pl_vals);
		fe_face_neighbor.get_function_values(temp_pl_solution_kplus1, pl_vals_neighbor);

		fe_face.get_function_gradients(temp_pl_solution_kplus1, pl_grads);
		fe_face_neighbor.get_function_gradients(temp_pl_solution_kplus1, pl_grads_neighbor);

		fe_face.get_function_values(temp_Sa_solution_kplus1, Sa_vals);
		fe_face_neighbor.get_function_values(temp_Sa_solution_kplus1, Sa_vals_neighbor);
		...
		for (unsigned int point = 0; point < n_qpoints; ++point)
		{
			...
			double Sa_nplus1_extrapolation0 = Sa_value0;
			double Sa_nplus1_extrapolation1 = Sa_value1;
			...
			double coef0_diff = lambda_v0*kappa0*dpcv_dSv0;
			double coef1_diff = lambda_v1*kappa1*dpcv_dSv1;
		    ...
		    double coef0_Sv_stab = (-lambda_v0*dpcv_dSv0+Kappa_tilde_v)*kappa0;
            double coef1_Sv_stab = (-lambda_v1*dpcv_dSv1+Kappa_tilde_v)*kappa1;
			...
			for (unsigned int i = 0; i < n_dofs; ++i)
			{
				...
				if(project_only_kappa)
				{
					coef0_darcy = lambda_v0;
					coef1_darcy = lambda_v1;
				}
				else
				{
					coef0_darcy = lambda_v0/lambda_t0;
					coef1_darcy = lambda_v1/lambda_t1;
				}

				double Dg0 = kappa0*rho_v0*lambda_v0;
				double Dg1 = kappa1*rho_v1*lambda_v1;
				...
				if(!project_Darcy_with_gravity)
				{
					coef0_g = kappa0*rho_v0*lambda_v0;
					coef1_g = kappa1*rho_v1*lambda_v1;
...
template <int dim>
void VaporSaturationProblem_midpoint<dim>::solve_vapor_saturation()
{
	...
	{
		SolverControl solver_control(Sv_solution.size(), 1.e-7 * right_hand_side_vapor_saturation.l2_norm());
