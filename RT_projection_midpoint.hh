#ifndef RT_PROJECTION_MID_HH_
#define RT_PROJECTION_MID_HH_
...
namespace RT_ProjectionMidpoint
{
using namespace dealii;
//using namespace FE_RTProj;

template<int dim>
PETScWrappers::MPI::Vector compute_RT0_projection_midpoint(Triangulation<dim, dim> &triangulation, const unsigned int degree, double theta_pl, double time,
		...
		bool use_exact_Sa_in_RT0, bool use_exact_Sv_in_RT0, bool second_order_extrapolation,
		PETScWrappers::MPI::Vector pl_solution_kplus1, PETScWrappers::MPI::Vector Sa_solution_k,
		PETScWrappers::MPI::Vector Sv_solution_k, PETScWrappers::MPI::Vector kappa_abs_vec,
		bool project_only_kappa, MPI_Comm mpi_communicator, const unsigned int n_mpi_processes, const unsigned int this_mpi_process)
{
	...
	temp_pl_solution_kplus1.reinit(locally_owned_dofs_DG,
							locally_relevant_dofs_DG,
							mpi_communicator);

	temp_Sa_solution_k.reinit(locally_owned_dofs_DG,
							  locally_relevant_dofs_DG,
							  mpi_communicator);

	temp_Sv_solution_k.reinit(locally_owned_dofs_DG,
							  locally_relevant_dofs_DG,
							  mpi_communicator);
		...
		if (cell_DG->subdomain_id() == this_mpi_process)
		{
			...
			for (const auto &face : cell_RT0->face_iterators())
			{
				...
				if(face->at_boundary())
				{
					...
					fe_face_values_DG.get_function_values(temp_pl_solution_kplus1, pl_vals_face);

					std::vector<double> Sa_vals_face_k(n_q_points_face_DG);
					fe_face_values_DG.get_function_values(temp_Sa_solution_k, Sa_vals_face_k);

					std::vector<double> Sv_vals_face_k(n_q_points_face_DG);
					fe_face_values_DG.get_function_values(temp_Sv_solution_k, Sv_vals_face_k);

					std::vector<Tensor<1, dim>> grad_pl_face(n_q_points_face_DG);
					fe_face_values_DG.get_function_gradients(temp_pl_solution_kplus1, grad_pl_face);
					...
					for (unsigned int q = 0; q < n_q_points_face_test_scalar; ++q)
					{
						...
						double Sa_value_k = Sa_vals_face_k[q];
						double Sv_value_k = Sv_vals_face_k[q];
...
//						}
						...
						double gamma_ch_e = lambda_t*kappa;
				...
				else
				{
					...
					fe_face_values_DG_neighbor.get_function_values(temp_pl_solution_kplus1, pl_vals_face_neighbor);

					std::vector<double> Sa_vals_face_k(n_q_points_face_DG);
					fe_face_values_DG.get_function_values(temp_Sa_solution_k, Sa_vals_face_k);

					std::vector<double> Sa_vals_face_k_neighbor(n_q_points_face_DG);
					fe_face_values_DG_neighbor.get_function_values(temp_Sa_solution_k, Sa_vals_face_k_neighbor);

					std::vector<double> Sv_vals_face_k(n_q_points_face_DG);
					fe_face_values_DG.get_function_values(temp_Sv_solution_k, Sv_vals_face_k);

					std::vector<double> Sv_vals_face_k_neighbor(n_q_points_face_DG);
					fe_face_values_DG_neighbor.get_function_values(temp_Sv_solution_k, Sv_vals_face_k_neighbor);

					std::vector<Tensor<1, dim>> grad_pl_face(n_q_points_face_DG);
					fe_face_values_DG.get_function_gradients(temp_pl_solution_kplus1, grad_pl_face);

					std::vector<Tensor<1, dim>> grad_pl_face_neighbor(n_q_points_face_DG);
					fe_face_values_DG_neighbor.get_function_gradients(temp_pl_solution_kplus1, grad_pl_face_neighbor);

					double kappa0 = temp_kappa[cell_DG->active_cell_index()];
					double kappa1 = temp_kappa[cell_DG_neighbor->active_cell_index()];
					...
					for (unsigned int q = 0; q < n_q_points_face_test_scalar; ++q)
					{
						...
						double Sa_value0_k = Sa_vals_face_k[q];
						double Sa_value1_k = Sa_vals_face_k_neighbor[q];

						double Sv_value0_k = Sv_vals_face_k[q];
						double Sv_value1_k = Sv_vals_face_k_neighbor[q];
...
//						}
						...
						double coef0 = lambda_t0*kappa0;
						double coef1 = lambda_t1*kappa1;
...
template<int dim>
PETScWrappers::MPI::Vector compute_RT0_projection_with_gravity_midpoint(Triangulation<dim, dim> &triangulation,
...
template<int dim>
PETScWrappers::MPI::Vector compute_RTk_projection_midpoint(Triangulation<dim, dim> &triangulation, const unsigned int degree, double theta_pl, double time,
		double time_step, double penalty_pl, double penalty_pl_bdry, std::vector<unsigned int> dirichlet_id_pl, bool use_exact_pl_in_RT0,
		bool use_exact_Sa_in_RT0, bool use_exact_Sv_in_RT0, bool second_order_extrapolation,
		PETScWrappers::MPI::Vector pl_solution_kplus1, PETScWrappers::MPI::Vector Sa_solution_k,
		PETScWrappers::MPI::Vector Sv_solution_k, PETScWrappers::MPI::Vector kappa_abs_vec,
...
//		if (cell_DG->subdomain_id() == this_mpi_process)
	...
	//		sol_all_faces_one_cell = 0.0;
			...
			fe_values_DG.get_function_values(temp_pl_solution_kplus1, pl_vals_cell);

			std::vector<double> Sa_vals_cell_k(n_q_points_DG);
			fe_values_DG.get_function_values(temp_Sa_solution_k, Sa_vals_cell_k);

			std::vector<double> Sv_vals_cell_k(n_q_points_DG);
			fe_values_DG.get_function_values(temp_Sv_solution_k, Sv_vals_cell_k);

			std::vector<Tensor<1, dim>> grad_pl_cell(n_q_points_DG);
			fe_values_DG.get_function_gradients(temp_pl_solution_kplus1, grad_pl_cell);
			...
			for (unsigned int q = 0; q < n_q_points_cell_test_vector; ++q)
			{
				...
				double Sa_value_k = Sa_vals_cell_k[q];
				double Sv_value_k = Sv_vals_cell_k[q];
...
template<int dim>
PETScWrappers::MPI::Vector compute_RTk_projection_with_gravity_midpoint(Triangulation<dim, dim> &triangulation, const unsigned int degree, double theta_pl, double time,
		double time_step, double penalty_pl, double penalty_pl_bdry, std::vector<unsigned int> dirichlet_id_pl, bool use_exact_pl_in_RT0,
		bool use_exact_Sa_in_RT0, bool use_exact_Sv_in_RT0, bool second_order_extrapolation,
		PETScWrappers::MPI::Vector pl_solution, PETScWrappers::MPI::Vector Sa_solution_n, PETScWrappers::MPI::Vector Sa_solution_nminus1,
		PETScWrappers::MPI::Vector Sv_solution_n, PETScWrappers::MPI::Vector Sv_solution_nminus1, PETScWrappers::MPI::Vector kappa_abs_vec,
