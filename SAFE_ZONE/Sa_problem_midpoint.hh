#ifndef SA_PROBLEM_MID_HH
#define SA_PROBLEM_MID_HH
...
namespace AqueousSaturationMidpoint
{
...
template <int dim>
class AqueousSaturationProblem_midpoint
{
public:
	AqueousSaturationProblem_midpoint(Triangulation<dim, dim> &triangulation_,
			const unsigned int degree_, double time_step_, double theta_n_time_,
			double theta_Sa_, double penalty_Sa_,
			...
			bool use_direct_solver_, bool Stab_a_, bool project_Darcy_with_gravity_,
...
template <int dim>
AqueousSaturationProblem_midpoint<dim>::AqueousSaturationProblem_midpoint(Triangulation<dim, dim> &triangulation_,
		const unsigned int degree_, double time_step_, double theta_n_time_,
		double theta_Sa_, double penalty_Sa_,
		double penalty_Sa_bdry_, std::vector<unsigned int> dirichlet_id_sa_, bool use_exact_pl_in_Sa_,
		bool use_exact_Sv_in_Sa_, double time_, unsigned int timestep_number_,
...
template <int dim>
void AqueousSaturationProblem_midpoint<dim>::setup_system()
{
...
template <int dim>
void AqueousSaturationProblem_midpoint<dim>::assemble_system_matrix_aqueous_saturation()
{
	setup_system();
								 ...
								 auto & copy_data)
	{
		...
		double maximum_Sa = *std::max_element(old_Sa_vals_k.begin(), old_Sa_vals_k.end());
...
//			double rho_a_nminus1 = rho_a_fcn.value(pl_value_nminus1);
			...
			for (unsigned int i = 0; i < n_dofs; ++i)
			{
				...
				if(timestep_number == 1 || !second_order_time_derivative)
				{
					copy_data.cell_rhs(i) += (1.0/(theta_n_time*time_step)) * phi_nplus1 * Sa_value_n
							* fe_v.shape_value(i, point) * JxW[point];
				}
				else
				{
//					copy_data.cell_rhs(i) += (1.0/time_step)
//											* (2.0 * phi_nplus1 * rho_a * Sa_value_n
//													- 0.5 * phi_nplus1 * rho_a * Sa_value_nminus1)
//											* fe_v.shape_value(i, point) * JxW[point];
				}
				// Sv term
				copy_data.cell_rhs(i) += lambda_a * dpca_dSv * kappa * Sv_grad_nplus1_extrapolation
				...
				if(project_only_kappa)
					copy_data.cell_rhs(i) += (lambda_a) * totalDarcyVelo_extrapolation
							* fe_v.shape_grad(i, point) * JxW[point];
				else
					copy_data.cell_rhs(i) += (lambda_a/lambda_t) * totalDarcyVelo_extrapolation
						* fe_v.shape_grad(i, point) * JxW[point];

				// Gravity term
				if(!project_Darcy_with_gravity)
					copy_data.cell_rhs(i) += kappa*rho_a*lambda_a
									 ...
									 auto & copy_data)
	{
		...
		if(dirichlet)
		{
			for (unsigned int point = 0; point < n_qpoints; ++point)
			{
				...
				double gamma_Sa_e = fabs(lambda_a*kappa*dpca_dSa);
				...
				for (unsigned int i = 0; i < n_facet_dofs; ++i)
				{
					...
					if(project_only_kappa)
						...
						copy_data.cell_rhs(i) -= (lambda_a/lambda_t)
											* totalDarcyVelo_extrapolation
											* normals[point]
											* fe_face.shape_value(i, point)
											* JxW[point];
...
//					}
								 ...
								 auto & copy_data)
	{
		...
		double maximum_Sa0 = *std::max_element(old_Sa_vals_k.begin(), old_Sa_vals_k.end());
		double maximum_Sa1 = *std::max_element(old_Sa_vals_k_neighbor.begin(), old_Sa_vals_k_neighbor.end());
		...
		for (unsigned int point = 0; point < n_qpoints; ++point)
		{
			...
			double coef0_diff = fabs(lambda_a0*kappa0*dpca_dSa0);
			...
			double coef0_Sv = lambda_a0*dpca_dSv0*kappa0;
			double coef1_Sv = lambda_a1*dpca_dSv1*kappa1;
			...
			for (unsigned int i = 0; i < n_dofs; ++i)
			{
				...
				if(project_only_kappa)
				{
					coef0_darcy = lambda_a0;
					coef1_darcy = lambda_a1;
				}
				else
				{
					coef0_darcy = lambda_a0/lambda_t0;
					coef1_darcy = lambda_a1/lambda_t1;
				}

				double Dg0 = kappa0*rho_a0*lambda_a0;
				double Dg1 = kappa1*rho_a1*lambda_a1;
				...
				if(!project_Darcy_with_gravity)
				{
					coef0_g = kappa0*rho_a0*lambda_a0;
					coef1_g = kappa1*rho_a1*lambda_a1;
...
template <int dim>
void AqueousSaturationProblem_midpoint<dim>::solve_aqueous_saturation()
{
	...
	{
		SolverControl solver_control(Sa_solution.size(), 1.e-7 * right_hand_side_aqueous_saturation.l2_norm());
