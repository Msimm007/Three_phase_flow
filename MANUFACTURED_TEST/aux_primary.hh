...
// ----------------- Global variables (declared extern here) -----------------
extern bool inc;

extern double porosity_data;
extern double kappa;

extern double rho_l_data;
extern double rho_a_data;
extern double rho_v_data;

extern double mu_l_data;
...
// Mesh creator
template <int dim>
void create_mesh(Triangulation<dim, dim> &triangulation,
                 ...
                 std::vector<unsigned int> &dirichlet_id_sv)
{
    ...
    for (; cell != endc; cell++)
    {
        for (unsigned int face_no = 0; face_no < GeometryInfo<dim>::faces_per_cell; face_no++)
        {
            ...
            {
                // Placeholder for boundary tagging if needed
            }
        }
    }
...
// Initial Sa vector
template <int dim>
void create_initial_Sa_vector(Triangulation<dim, dim> & /*triangulation*/,
...
{
    if (dim == 3)
        return (2.0 - p[0]*p[0]*p[1]*p[1]*p[2]*p[2]*p[2]*p[2])/4.0;
    else    
                                     ...
                                     const unsigned int /*component*/) const
{
    ...
    if (dim == 3){
        grad_Sl[0] = -p[0]*p[1]*p[1]*p[2]*p[2]*p[2]*p[2]/2.0;
        grad_Sl[1] = -p[0]*p[0]*p[1]*p[2]*p[2]*p[2]*p[2]/2.0;        
        grad_Sl[2] = -p[0]*p[0]*p[1]*p[1]*p[2]*p[2]*p[2];        
    }
    else {
